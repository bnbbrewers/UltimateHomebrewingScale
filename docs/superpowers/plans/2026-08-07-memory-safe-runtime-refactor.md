...
- Create `netcore/http_transport.py`: shared HTTP request, retry, response-spooling, status, close, and GC policy without importing the LVGL-heavy `core` package or conflicting with MicroPython's `network` module.
...
**Files:** `netcore/http_transport.py`, `updater/http_client.py`, `api/brewfather_api.py`, `updater/workflow.py`, `updater/github_release.py`
...
- [ ] **Step 2: Implement `netcore/http_transport.py`**
...
```powershell
rtk git add netcore/http_transport.py updater/http_client.py api/brewfather_api.py api/brewing_software_api.py updater/workflow.py updater/github_release.py
