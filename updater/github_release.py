...
def resolve_update_step(
    channel, local_version, requests_module=None, i18n=None, manifest_loader=None
):
    """Resolve the update to apply, or None when already up to date.

    There is no chain: a release ships the complete runtime, so the newest
    release the channel can see is always reachable in one step.
    """
    requests_module = requests_module or http_client.default_requests_module()
    if requests_module is None:
        raise RuntimeError("Missing requests2 module")

    current = resolve_release(channel, requests_module=requests_module, i18n=i18n)
    local = str(local_version or "").strip()
    tag = current.get("tag", "")
    if local and local == tag:
        return None

    if manifest_loader is None:
        def manifest_loader(url):
            return download_manifest(url, requests_module=requests_module, i18n=i18n)

    manifest = manifest_loader(current.get("manifest_url", ""))
    ...
    return {"release": current, "manifest": manifest}
